...
async function resolvePrivateKeyContent(body: ImportBody): Promise<{ ok: true; content: string } | { ok: false; error: string }> {
  ...
  try {
    ...
    sshpk.parsePrivateKey(body.privateKey, 'auto', body.passphrase ? { passphrase: body.passphrase } : undefined);
  } catch (err) {
...
function secretData(privateKeyContent: string, passphrase: string | undefined): Record<string, string> {
  const data: Record<string, string> = { 'ssh-privatekey': Buffer.from(privateKeyContent).toString('base64') };
...
export function registerSSHKeyImportRoutes(app: AnyElysia): AnyElysia {
  return app
    .post('/api/v1/namespaces/:namespace/ansiblesshkeys/import', async ({ params, body, headers, set }) => {
      ...
      return client.create(
        RESOURCE_DESCRIPTORS_BY_KIND.AnsibleSSHKey!,
        {
          ...
          spec: {
            ...
            ...(importBody.passphrase ? { passphraseSecretRef: { name: importBody.name, key: 'passphrase' } } : {}),
          },
        },
        session.identity,
        params.namespace,
    ...
    .post('/api/v1/clusteransiblesshkeys/import', async ({ body, headers, set }) => {
      ...
      if (!importBody.secretNamespace) {
        set.status = 400;
        return { error: 'secretNamespace is required for a ClusterAnsibleSSHKey (no owning namespace to default to)' };
      }
      ...
      return client.create(
        RESOURCE_DESCRIPTORS_BY_KIND.ClusterAnsibleSSHKey!,
        {
          ...
          metadata: { name: importBody.name },
          spec: {
            secretRef: { name: importBody.name, namespace: importBody.secretNamespace, key: 'ssh-privatekey' },
            ...(importBody.passphrase
              ? { passphraseSecretRef: { name: importBody.name, namespace: importBody.secretNamespace, key: 'passphrase' } }
              : {}),
          },
        },
        session.identity,
