...
<template>
  <div style="width: 100%">
    <div v-for="(entry, i) in matchLabelEntries" :key="i" style="display: flex; gap: 8px; margin-bottom: 8px">
      <el-input :model-value="entry[0]" placeholder="key" @update:model-value="(v: string | number) => updateLabelKey(i, String(v))" />
      <el-input :model-value="entry[1]" placeholder="value" @update:model-value="(v: string | number) => updateLabelValue(i, String(v))" />
      <el-button @click="removeLabel(i)">Remove</el-button>
    </div>
